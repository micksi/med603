...
Shader "Custom/DLAA" 
{
	...
	float4 highPassPre( float2 texCoord )
	{
		...
		LD(float4 sUpLeft, -1.0 * _OffsetSize,-1.0 * _OffsetSize)
		LD(float4 sUpRight, 1.0 * _OffsetSize,-1.0 * _OffsetSize)
		LD(float4 sDownLeft, -1.0 * _OffsetSize,1.0 * _OffsetSize)
		LD(float4 sDownRight, 1.0 * _OffsetSize,1.0 * _OffsetSize)
