...
Shader "Custom/OurDLAA" 
{
	...
	float4 edgeDetectAndBlur(  float2 texCoord )
	{
		...
		float4 clr = center;
		clr = lerp( clr, blurred_h, edge_mask_v  * csf /** _Amount*/);
		clr = lerp( clr, blurred_v, edge_mask_h  * csf /** _Amount*/); // blurrier version
		...
		//if ( abs( long_edge_mask_h - long_edge_mask_v ) > 0.2f ) // resistant to noise (TFU2 SPUs)
		{
			...
			clr = lerp( clr, clr_v, long_edge_mask_v  * csf /** _Amount*/);
			clr = lerp( clr, clr_h, long_edge_mask_h  * csf /** _Amount*/);
		}
